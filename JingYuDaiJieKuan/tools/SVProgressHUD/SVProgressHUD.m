...
static const CGFloat SVProgressHUDRingNoTextRadius = 15;
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        SVProgressHUDBackgroundColor = [UIColor colorWithWhite:0.f alpha:0.8f];
...
- (void)updatePosition {
    ...
    CGFloat hudWidth = 70.0f;
    CGFloat hudHeight = 70.0f;
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    self.hudView.layer.borderColor = [UIColor blackColor].CGColor;
    self.hudView.layer.borderWidth = 0.5f;
