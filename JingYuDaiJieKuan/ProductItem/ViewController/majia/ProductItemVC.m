...
- (void)initView{
    ...
    self.m_productHeadView.frame = CGRectMake(0, CGRectGetMaxY(self.contentView.frame)+13 , kScreenWidth, kiP6WidthRace(44));
    
    [self.tipView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.top.equalTo(self.searchBGView.mas_bottom).offset(kiP6WidthRace(44)+13);
//        make.top.equalTo(self.m_productHeadView.mas_bottom);
    ...
    [ProductItemViewModel productItemListPath:productItemListPath params:params target:self success:^(ProductItemListModel *model) {
        if (model.code == 200) {
            if (weakSelf.pageIndex == 1) {
                weakSelf.simpleLoanArray = model.result;
...
- (void)getRandomMerchant {
    ...
    [ProductItemViewModel randomProductPath:path params:nil target:weakSelf success:^(randomProductModel *model) {
        if (model.code == 200) {
            ...
            [mustrAtt addAttribute:NSForegroundColorAttributeName value:UIColorHex(@"#DC1D1D") range:NSMakeRange(blackStr.length, AllStr.length - blackStr.length)];
...
- (UIView *)searchBGView
{
    if (!_searchBGView) {
        ...
        //切圆角
        _searchBGView.backgroundColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0];
//        _searchBGView.layer.cornerRadius = 5;
//        _searchBGView.layer.masksToBounds = true;
        _searchBGView.layer.cornerRadius = 15.5;
        _searchBGView.layer.shadowColor =  [UIColor colorWithRed:238/255.0 green:239/255.0 blue:242/255.0 alpha:1.0].CGColor;
        _searchBGView.layer.shadowOffset = CGSizeMake(0,0);
        _searchBGView.layer.shadowOpacity = 1;
        _searchBGView.layer.shadowRadius = 10;
...
- (ProductItemHeadView *)m_productHeadView
{
    if (!_m_productHeadView) {
        ...
        _m_productHeadView.showFilterBlock = ^(BOOL isSelect) {
            if (isSelect) {
                weakSelf.m_productHeadView.frame = CGRectMake(0, CGRectGetMaxY(weakSelf.contentView.frame)+13, kScreenWidth, kiP6WidthRace(44) + kiP6WidthRace(820/2));
            }else{
                weakSelf.m_productHeadView.frame = CGRectMake(0, CGRectGetMaxY(weakSelf.contentView.frame)+13, kScreenWidth, kiP6WidthRace(44));
            }
        };
...
{
    if (!_tableView) {
        _tableView = InsertTableView(nil, CGRectMake(0, 0, kScreenWidth, CGRectGetHeight(self.view.frame)), self, self, UITableViewStylePlain, UITableViewCellSeparatorStyleNone);
