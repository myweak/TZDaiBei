...
#pragma mark toolbar图片操作代理方法
- (void)toolbarActionSheetSelectType:(ImageActionSheetSelectType)type image:(UIImage *)image
{
    [self hide];
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowserImageOperateType:image:)]) {
        [_delegate photoBrowserImageOperateType:type image:image];
    }
    
}

#pragma mark photoView图片操作代理方法
-(void)photoViewActionSheetSelectType:(ImageActionSheetSelectType)type image:(UIImage *)image
{
    [self hide];
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowserImageOperateType:image:)]) {
        [_delegate photoBrowserImageOperateType:type image:image];
