...
@interface MJPhotoView ()<UIActionSheetDelegate>
{
    ...
    CGFloat _lastScale;  // by 彭坤才 20140725
}
@end
...
//        NSLog(@"长按  图片：%@",[_photo.url absoluteString]);
...
//    [self performSelector:@selector(showNavigationBar)];
...
//    if (buttonIndex == 0) {
//        
//        if (_photoViewDelegate && [_photoViewDelegate respondsToSelector:@selector(photoViewActionSheetSelectType:image:)]) {
//            [_photoViewDelegate photoViewActionSheetSelectType:SendToHXCircle image:_photo.image];
//        }
//        
//    }else
        if(buttonIndex == 0) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    if (error) {
        ...
        [[ZXAlertView shareView] showMessage:@"保存失败"];

    } else {
        _photo.save = YES;
        [[ZXAlertView shareView] showMessage:@"成功保存到相册"];
    }
}
    ...
    if (_photo.image) {
        ...
        [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            if (receivedSize > kMinProgress) {
                ...
                if (loading && [loading respondsToSelector:@selector(setProgress:)]) {
